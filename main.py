    ...
    if uploaded_file and st.button("Generate Appeal"):
        with st.spinner('Agents are working...'):
            analyst = Agent(role='Analyst',
                            goal='Extract Details',
                            backstory='Expert Biller',
                            verbose=True)
            writer = Agent(role='Writer',
                           goal='Write Appeal',
                           backstory='Legal Expert',
                           verbose=True)

            raw_text = get_pdf_text(uploaded_file)
            task1 = Task(description=f"Analyze: {raw_text}",
                         expected_output='Details',
                         agent=analyst)
            task2 = Task(description="Write appeal with Markdown.",
...
with tab2:
    ...
    medical_image = st.file_uploader("Upload Scan",
                                     type=["jpg", "png", "jpeg"])
